...
Status: **S17 PLAN / RESEARCH REVIEW**

## Completed
- BOR-S0–S16 are merged to `main`.
- `main` commit `035cfada8b57bbeb0c5db54c22cefec288dfb868` passed BLACK ORACLE REPORT CI #73.
- S15 provides the canonical integrity-gated `bor.alpha-read-model.v1`; S16 provides GET-only `bor.alpha-read-api.v1` serialization.
...
- non-GET request remains `405 METHOD_NOT_ALLOWED`
...
### Rollback
Repository-only revert of the S17 branch/PR. S0–S16 canonical artifacts, API contract, runtime health/version endpoints, database schema, and deployment configuration remain unchanged.

### Research review / constraints
DI-001/003/004, AIML-005/006, BOR-S8-E1 through BOR-S16-E1 constrain S17. S15/S16 establish that UI/runtime transports are consumers of verified versioned artifacts, not synthesis layers. BOR-S17-H1: dependency-injected route composition can expose S16 without widening authority. BOR-S17-E1 will test that hypothesis against deterministic route/integrity cases.

### Exact next gate
Complete S17 research record → implement route composition → typecheck/build/deterministic tests → verify exact model/citation/scenario/uncertainty preservation and no-authority boundary → document result → open PR and require green CI before merge.
...
- Single next priority: implement and verify BOR-S17-E1 without changing frozen Alpha scope.
